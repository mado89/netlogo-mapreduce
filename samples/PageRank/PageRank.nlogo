...
to map-outlinks [#key #values]
  ...
  ;;; now loop over all outlinks and 
  foreach bf bf #values [
    mapreduce:emit site 1
  ]
end

to extractrank [#key #values]
  let site first #values
  let rank first bf #values

  mapreduce:emit site rank
end

to-report identity [#key #accum #value]
  report #value
end

to pre-process [#input]
  mapreduce:config.valueseparator " "
  let res1 mapreduce:mapreduce "map-outlinks" "summer" 0 #input
  while [mapreduce:running?] [wait 1 tick]
  tick
  let res2 mapreduce:mapreduce "extractrank" "identity" 0 #input
  while [mapreduce:running?] [wait 1 tick]
  tick
  set outlinks table:from-list mapreduce:result res1
  set ranks table:from-list mapreduce:result res2
  show "Pre-Processing done"
  show ranks
end

to-report #outlinks [#page]
  report table:get outlinks #page
end

to iteration [#input]
  mapreduce:config.valueseparator " "
  let res mapreduce:mapreduce "distributerank" "summer" 0 #input
  while [mapreduce:running?] [wait 1 tick]
  tick
