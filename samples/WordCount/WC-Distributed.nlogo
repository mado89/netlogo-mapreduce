...
First one model needs to run as server. Therefore press the server button (starts a HubNet activity).
On the other computers also load the model (not the HubNet-client) and press node.
Once enough nodes/clients have joined the activity select the data-set you want to use and press "Count Words".
