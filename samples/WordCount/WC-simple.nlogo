...
to read-file [file-name line]
  ...
  [
    ; show ?
    ;;;  emit: <word, 1> the word is reported by splid
    mapreduce:emit ? "1"
  ]
end
...
to count-words
  ...
  let res mapreduce:mapreduce "read-file" "sum-occurrences" 0 data-set
...
This example should be the starting example when MapReduce is introduced. In case students
are not familiar with multi-threading or parallel programming it can be presented in a way that
the students first have to create a program counting the words in an already familiar manner like
a plain iterative program (check out the iterative word count model). 

What the students should take from the presentation is that with using MapReduce a simple
task like counting all words in a set of files can be done in a very easy way with just a few lines
of code.

## HOW IT WORKS

The model just starts a simple MapReduce job counting the words of the documents in a certain directory. 
The mapper is read-file.
The reducer is sum-occurrences.

## HOW TO USE IT

Select the data-set you want to use and press "Count Words"
...
When using the normal input-reading procedure a map-task always receives a line as value. For the word-count example this line is then split up within NetLogo into individual words. However, the framework also offers the possibility to configure a value-separator. This feature can be used to split up a line into individual words before passing it to a map-task. Instead of a string the mapper now receives a list of strings.
...
This model uses code taken from http://groups.yahoo.com/group/netlogo-users/message/6490
