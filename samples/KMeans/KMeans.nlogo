...
input
input
"points.txt" "points2.txt" "points3.txt" "points4.txt"
3
...
K-Means clustering or simple k-means is a method using vector quantization for cluster analysis.
In machine learning or data mining a cluster is a group of objects sharing certain properties. The algorithm is used to partition a data set of size n into k clusters where every data point belongs to the cluster with the nearest mean. This mean or center point does not have to be in the input data set. 
K-Means clustering can be applied to all sorts of data as long as there exist a function for measuring the distance between to vectors. In a simple case k-means can be applied to two dimensional vectors i.e. points on a plane. 
The algorithm then computes k groups of points belonging to the same region. Of course this problem can not be solve easily (computationally
wise) since the algorithm involve a repetitive computation of distance to all the center points for
all data points.

## HOW IT WORKS

In the map step the algorithm computes a list: For every point the nearest center is determine
In the reduce step for every center all points belonging to it are used to compute the new coordinates of the center
The center points are then updated using the new data.
...
Try to hit init several times to get better inital center points
Try to perform k iterations again
