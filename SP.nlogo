...
to shortest-path
  ...
  while [notfinished?] [
    ...
    while [mapreduce:running?] [wait 1]
...
This model creates a directed graph

## HOW IT WORKS

When in node-creation mode nodes are created upon a click on world. When a node is clicked on for a second time it is deleted
When in edge-creation mode an edge is created after a clicks on distinct nodes

Make-Input creates a list for every node all its connected nodes.
@#$#@#$#@
default
true
0
