...
all: mapreduce/mapreduce.jar mapreduce/client-lib.jar
...
mapreduce/mapreduce.jar: $(SRCS) manifest.txt # mapreduce/framework.jar
	mkdir -p classes
	$(JAVAC) -g -encoding us-ascii -source 1.5 -target 1.5 -classpath $(NETLOGO)/NetLogo.jar:/usr/share/java/log4j-1.2.jar:mapreduce/client-lib.jar -d classes $(SRCS)
